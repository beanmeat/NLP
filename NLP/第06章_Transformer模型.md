...
>在 Transformer 的 Self-Attention 中，会先计算 Query 和 Key 的相似度： `score = QKᵀ` 然后通过 softmax 得到注意力权重： `Attention = softmax(QKᵀ / √dₖ)V` 其中 `√dₖ` 就是缩放因子。 原因如下： 
>
>1. **防止点积结果过大** Q 和 K 的维度通常比较高，例如 `dₖ=512`。点积本质上是多个维度相乘后求和： `q·k=q₁k₁+q₂k₂+...+q₅₁₂k₅₁₂` 随着维度增加，点积结果的数值会变大。 如果直接将较大的点积结果输入 softmax，会导致 softmax 输出过于极端，例如： `[100,50,20] → [1,0,0]` 模型几乎只关注一个位置，其他位置的信息会被忽略。  
>2. **避免 softmax 饱和，保证梯度稳定** 当 softmax 输入过大时，输出接近 0 或 1，此时梯度会变得非常小。 梯度过小会导致反向传播时参数更新困难，模型训练速度下降。  
>3. **为什么除以 √dₖ？** 因为点积结果的方差会随着维度 `dₖ` 增大而增大，其标准差约为 `√dₖ`。 因此除以 `√dₖ` 可以将点积结果缩放到合理范围，使不同维度下的注意力分数保持稳定。  总结： 注意力点积缩放的目的，是防止高维 Query 和 Key 的点积过大，导致 softmax 输出过于尖锐以及梯度消失问题。除以 `√dₖ` 可以稳定注意力分布，使 Transformer 更容易训练。

**Transformer为什么用LayerNorm？**

>Transformer 中使用 LayerNorm（Layer Normalization）的主要目的是 **稳定训练过程，加快模型收敛，并避免深层网络中的梯度问题**。
>
>Transformer 的每一层通常包含： 
>
>- Multi-Head Self-Attention
>- Feed Forward Network（FFN）
>
>在这些模块之后都会使用 LayerNorm：`x → Attention → Add → LayerNorm`,`x → FFN → Add → LayerNorm`
>
>1. **防止网络层之间的数据分布发生变化** 在 Transformer 训练过程中，参数不断更新，每一层输出的数据分布也会不断变化，例如：某一层输出：`[1.2, 3.5, 10.8, 0.5]`经过训练后可能变成：`[100, 300, 800, 50]`这种数值范围不断变化，会导致后面的网络难以学习。LayerNorm 会对每个样本内部的特征进行归一化，使输出保持稳定的分布。
>
>2. **缓解梯度消失和梯度爆炸** Transformer 通常有很多层堆叠，例如：
>
>   - BERT：几十层 Encoder
>   - GPT：几十甚至上百层 Decoder
>
>   如果没有归一化：前面的层梯度经过多层传播后可能：`变得非常小（梯度消失）`，`变得非常大（梯度爆炸）`LayerNorm 可以让每层输入保持稳定，使梯度更容易传播。
...
nn.Transformer 封装了完整的前向传播逻辑，其 forward() 方法定义了编码器 -> 解码器的执行流程。该函数接收源语言序列（src_sequence，编码器输入）和目标语言序列（tgt_sequence，解码器输入）作为输入，以解码器预测结果作为输出。如下图所示：
