...
![image-20260713132220261](images/image-20260713132220261.png)
...
- **重置门（Reset Gate）**
  ...
  ![image-20260713132318218](images/image-20260713132318218.png)

  计算公式如下：

  ![image-20260713132424866](images/image-20260713132424866.png)

  重置门会在计算当前时间步信息（候选隐藏状态）时，作用在上一个时间步的隐藏状态，用于控制遗忘多少旧信息，如下图所示：

  ![image-20260713132446740](images/image-20260713132446740.png)

  当前时间步的信息（候选隐藏状态）的计算公式为：

  ![image-20260713132459500](images/image-20260713132459500.png)

- **更新门（Update Gate）**

  更新门也由上一时间步的隐藏状态和当前时间步的输入计算得到，如下图所示：

  ![image-20260713132716250](images/image-20260713132716250.png)

  计算公式为

  ![image-20260713133010816](images/image-20260713133010816.png)

  更新门会在计算当前时间步最终的隐藏状态 $h_t$ 时，分别作用在上一时刻的隐藏状态 $h_{t-1}$ 和当前新计算出的候选隐藏状态 $\tilde{h}_t$，用于控制保留多少旧信息，以及引入多少新信息。

  ![image-20260713133039568](images/image-20260713133039568.png)

  最终隐藏状态的计算公式为：

  ![image-20260713133052559](images/image-20260713133052559.png)

**完整结构如下下图所示**

![image-20260713133331061](images/image-20260713133331061.png)

#### 3.3.3 多层结构

GRU同样支持多层结构

![image-20260713183122439](images/image-20260713183122439.png)

#### 3.3.4 双向结构

GRU同样支持双向结构

![image-20260713183138473](images/image-20260713183138473.png)

#### 3.3.5 多层+双向结构

GRU同样支持多层和双向结构

![image-20260713183155205](images/image-20260713183155205.png)
...
| input  | (seq_len, batch_size, input_size)                      | 输入序列，如果 `batch_first=True`，则为 (batch_size, seq_len, input_size) |
| h_0    | (num_layers × num_directions, batch_size, hidden_size) | 可选，初始隐藏状态                                           |
| output | (seq_len, batch_size, num_directions × hidden_size)    | RNN 层的输出，包含最后一层每个时间步的隐藏状态，如果 `batch_first=True`，则为 (batch_size, seq_len, num_directions × hidden_size) |
| h_n    | (num_layers × num_directions, batch_size, hidden_size) | 最后一个时间步的隐藏状态，包含每一层的每个方向               |

- 单层单向

  ![image-20260713134944036](images/image-20260713134944036.png)

- 多层单向

  ![image-20260713135008659](images/image-20260713135008659.png)

- 单层多向

  ![image-20260713135019513](images/image-20260713135019513.png)

- 多层多向

  ![image-20260713135036578](images/image-20260713135036578.png)
